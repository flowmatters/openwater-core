...
func (m *ConstituentDecay) ApplyParameters(parameters data.ND2[float64]) {
  ...
  m.halfLife = parameters.Slice([]int{ paramIdx, 0}, []int{ paramSize, nSets}, nil).MustReshape(newShape).(data.ND1[float64])
  paramIdx += paramSize
...
//  for i := 0; i < numCells; i++ {
  ...
  //    fmt.Println("{storage m^3}",tmpTS.Shape())
      storage := cellInputs.Slice([]int{ 4,0}, []int{ 1,inputLen}, nil).MustReshape(inputNewShape).(data.ND1[float64])
