...
func (m *BaseflowFilter) ApplyParameters(parameters data.ND2[float64]) {
...
func (m *BaseflowFilter) InitialiseDimensions(dims []int) {
  
}

func (m *BaseflowFilter) FindDimensions(parameters data.ND2[float64]) []int {
...
func (m *BaseflowFilter) Run(inputs data.ND3[float64], states data.ND2[float64], outputs data.ND3[float64]) {
...
//  for i := 0; i < numCells; i++ {
  ...
  //    fmt.Println("{streamflow <nil>}",tmpTS.Shape())
      streamflow := cellInputs.Slice([]int{ 0,0}, []int{ 1,inputLen}, nil).MustReshape(inputNewShape).(data.ND1[float64])
