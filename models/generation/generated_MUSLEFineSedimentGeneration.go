...
type MUSLEFineSedimentGeneration struct {
  gamma data.ND1Float64
  cr data.ND1Float64
  w data.ND1Float64
  a data.ND1Float64
  b1 data.ND1Float64
...
func (m *MUSLEFineSedimentGeneration) ApplyParameters(parameters data.ND2Float64) {
  ...
  m.gamma = parameters.Slice([]int{ paramIdx, 0}, []int{ paramSize, nSets}, nil).MustReshape(newShape).(data.ND1Float64)
  paramIdx += paramSize

  paramSize = 1
  newShape = []int{ nSets}

  m.cr = parameters.Slice([]int{ paramIdx, 0}, []int{ paramSize, nSets}, nil).MustReshape(newShape).(data.ND1Float64)
  paramIdx += paramSize

  paramSize = 1
  newShape = []int{ nSets}

  m.w = parameters.Slice([]int{ paramIdx, 0}, []int{ paramSize, nSets}, nil).MustReshape(newShape).(data.ND1Float64)
  paramIdx += paramSize
  ...
  m.latitude = parameters.Slice([]int{ paramIdx, 0}, []int{ paramSize, nSets}, nil).MustReshape(newShape).(data.ND1Float64)
  paramIdx += paramSize
...
func buildMUSLEFineSedimentGeneration() sim.TimeSteppingModel {
	result := MUSLEFineSedimentGeneration{}
	return &result
}

func init() {
	sim.Catalog["MUSLEFineSedimentGeneration"] = buildMUSLEFineSedimentGeneration
}

func (m *MUSLEFineSedimentGeneration)  Description() sim.ModelDescription{
  ...
  sim.DescribeParameter("gamma",0,"unitless",[]float64{ 0, 0 },"",gammaDims),
  sim.DescribeParameter("cr",0,"threshold cover",[]float64{ 0, 0 },"",crDims),
  sim.DescribeParameter("w",0,"weighting fraction between RUSLE and MUSLE approaches (1==RUSLE",[]float64{ 0, 1 }," ",wDims),
  sim.DescribeParameter("a",0,"",[]float64{ 0, 0 },"",aDims),
  sim.DescribeParameter("b1",0,"",[]float64{ 0, 0 },"",b1Dims),
  sim.DescribeParameter("b2",0,"",[]float64{ 0, 0 },"",b2Dims),
  sim.DescribeParameter("area",0,"[0",[]float64{ 0, 0 },"",areaDims),
  sim.DescribeParameter("latitude",0,"",[]float64{ 0, 0 },"",latitudeDims),
  sim.DescribeParameter("elevation",0,"",[]float64{ 0, 0 },"",elevationDims),
  ...
  "quickflow","baseflow","rainfall","cover","cFactor","month","Rcm",}
...
func (m *MUSLEFineSedimentGeneration) InitialiseDimensions(dims []int) {
  
}

func (m *MUSLEFineSedimentGeneration) FindDimensions(parameters data.ND2Float64) []int {
...
func (m *MUSLEFineSedimentGeneration) InitialiseStates(n int) data.ND2Float64 {
  // Zero states
...
func (m *MUSLEFineSedimentGeneration) Run(inputs data.ND3Float64, states data.ND2Float64, outputs data.ND3Float64) {
...
//	result.States = states  //clone? make([]sim.StateSet, len(states))
  ...
  // fmt.Println("Running MUSLEFineSedimentGeneration for ",numCells,"cells")
//  for i := 0; i < numCells; i++ {
  for j := 0; j < numCells; j++ {
    go func(i int){
      ...
      gamma := m.gamma.Get1(i%m.gamma.Len1())
      cr := m.cr.Get1(i%m.cr.Len1())
      w := m.w.Get1(i%m.w.Len1())
      a := m.a.Get1(i%m.a.Len1())
      b1 := m.b1.Get1(i%m.b1.Len1())
      b2 := m.b2.Get1(i%m.b2.Len1())
      area := m.area.Get1(i%m.area.Len1())
      latitude := m.latitude.Get1(i%m.latitude.Len1())
  ...
  //    fmt.Println("{cover [0,1] Visual Cover}",tmpTS.Shape())
      cover := cellInputs.Slice([]int{ 3,0}, []int{ 1,inputLen}, nil).MustReshape(inputNewShape).(data.ND1Float64)
      
  //    fmt.Println("{cFactor [0,1] Cover Factor}",tmpTS.Shape())
      cfactor := cellInputs.Slice([]int{ 4,0}, []int{ 1,inputLen}, nil).MustReshape(inputNewShape).(data.ND1Float64)
      
  //    fmt.Println("{month monthOfYear}",tmpTS.Shape())
      month := cellInputs.Slice([]int{ 5,0}, []int{ 1,inputLen}, nil).MustReshape(inputNewShape).(data.ND1Float64)
      
  //    fmt.Println("{Rcm [] Runoff Coefficient Metric}",tmpTS.Shape())
      rcm := cellInputs.Slice([]int{ 6,0}, []int{ 1,inputLen}, nil).MustReshape(inputNewShape).(data.ND1Float64)
      ...
      generatedloadcoarse := outputs.Slice(outputPosSlice,outputSizeSlice,outputStepSlice).MustReshape([]int{inputLen}).(data.ND1Float64)
       ...
       musleFine(quickflow,baseflow,rainfall,cover,cfactor,month,rcm,gamma,cr,w,a,b1,b2,area,latitude,elevation,avk,avls,avfines,dwc,maxconc,uslehsdrfine,uslehsdrcoarse,timestepinseconds,quickloadfine,slowloadfine,quickloadcoarse,slowloadcoarse,totalfineload,totalcoarseload,generatedloadfine,generatedloadcoarse)
