...
func (m *PassLoadIfFlow) ApplyParameters(parameters data.ND2[float64]) {
...
func (m *PassLoadIfFlow) FindDimensions(parameters data.ND2[float64]) []int {
...
func (m *PassLoadIfFlow) InitialiseStates(n int) data.ND2[float64] {
  // Zero states
...
func (m *PassLoadIfFlow) Run(inputs data.ND3[float64], states data.ND2[float64], outputs data.ND3[float64]) {
...
//  for i := 0; i < numCells; i++ {
  ...
  //    fmt.Println("{inputLoad <nil>}",tmpTS.Shape())
      inputload := cellInputs.Slice([]int{ 1,0}, []int{ 1,inputLen}, nil).MustReshape(inputNewShape).(data.ND1[float64])
      ...
      outputload := outputs.Slice(outputPosSlice,outputSizeSlice,outputStepSlice).MustReshape([]int{inputLen}).(data.ND1[float64])
